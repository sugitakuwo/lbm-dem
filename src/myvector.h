...
using namespace std;
enum ProblemName {NONE, SHEARCELL, SPLASH, AVALANCHE, DRUM, BOX, NET, DIFF, BARRIER, demChute, ZHOU, OPENBARRIER, HONGKONG, STAVA, CAROLINE};
...
public:
    ...
    tinyVector(const double& a, const double& b, const double& c){
        x=a;
        y=b;
        z=c;
    }
    ...
    bool insideBox(tVect center, double W, double H, double D) const;
...
public:
	...
	void reset();
	void print(std::ofstream& outputFile) const;
	void printNormal(std::ofstream& outputFile) const;
	void printShear(std::ofstream& outputFile) const;
    // overloaded operators
...
// list of types
class nodeType;
typedef std::vector<nodeType> typeList;
